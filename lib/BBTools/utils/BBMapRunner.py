...
class BBMapRunner:
    ...
    def run_local(self, io_params, app_params):
        ...
        result = {
            ...
            'run_log': run_log,
            'mapped_reads_file': None,
            'unmapped_reads_file': None,
            'bam_file': None,
            'run_command': run_command
        }
        output_created = False
        if 'mapped_reads_file' in file_lookup:
            result['mapped_reads_file'] = os.path.join(output_dir, file_lookup['mapped_reads_file'])
            output_created = True
        if 'unmapped_reads_file' in file_lookup:
            result['unmapped_reads_file'] = os.path.join(output_dir, file_lookup['unmapped_reads_file'])
            output_created = True
        if 'bam_file' in file_lookup:
            result['bam_file'] = os.path.join(output_dir, file_lookup['bam_file'])
            output_created = True
    ...
    def _process_app_params_to_cli(self, io_params, app_params, output_dir, run_log, is_app):
        ...
        available_params = {
            'out_mode': {'type': 'string', 
                         'allowed_values': ['split_reads', 'mapped_reads', 'unmapped_reads', 'bam']},
