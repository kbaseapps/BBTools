...
class RQCFilterRunner:
    ...
    def run_app(self, params):
        output_dir = self._run(params, is_app=True)
        return self._save_output_to_kbase(params, output_dir)

    def run_local(self, params):
        output_dir = self._run(params, is_app=False)
    ...
    def _process_app_params_to_cli(self, params, output_dir, is_app):
        ...
        self._validate(params, is_app)
        if 'read_library_ref' in params:
            reads_file = self._stage_input(params)['files']['fwd']
        else:
